...
class Sphere:
    ...
    @ti.func
    def init(self, o, x,y, m=1, v=vec2(0,0), ω=0):
        self.o = o
        self.x = x
        self.y = y
        self.m = m
        ...
        self.R = vec2(1,0)
        self.I = m * x * y /3.0
        self.size = ti.max(x,y)
        self.color = vec3(1,1,1)
    
    @ti.func
    def sdf(self, x):
        return ti.min(ti.abs(x - self.o).x-self.x, ti.abs(x - self.o).y-self.y)
    ...
    def sdf_grad(self, x):
        grad = vec2(0,0)
        if ti.abs(x - self.o).x - self.x < ti.abs(x - self.o).y - self.y:
            grad = vec2(-tm.sign((x - self.o).x), 0)
        else:
            grad = vec2(0, -tm.sign((x - self.o).y))
        return grad
