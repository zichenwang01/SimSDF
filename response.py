...
class CollisionResponse:
    ...
    def init_contact(self):
        ...
        self.N = self.scene.N
        self.collisions = Collision.field(shape=(self.N*self.N+4*self.N,))
        self.nc[None] = 0
    ...
    def addContact(self, p1: vec2, r1: vec2, r2: vec2, n1: vec2, i1: int, i2: int, sep: float, nc: int):
        ...
        if i1 >= 0:
            v += self.scene.boxes[i1].v + crossZ(self.scene.boxes[i1].ω, r1)
        if i2 >= 0:
            v -= self.scene.boxes[i2].v + crossZ(self.scene.boxes[i2].ω, r2)
    ...
    def initSums(self):
        ...
        for k in range(self.nc[None]):
            ...
            if C.i1 >= 0:
                d_va = C.gamma * C.n1
                d_wa = C.r1.cross(d_va)
                ti.atomic_add(self.gamma_x[C.i1], d_va)
                ti.atomic_add(self.sum_w[C.i1], d_wa)
            if C.i2 >= 0:
                d_vb = -C.gamma * C.n1
                d_wb = C.r2.cross(d_vb)
                ti.atomic_add(self.gamma_x[C.i2], d_vb)
                ti.atomic_add(self.sum_w[C.i2], d_wb)

        for i in range(self.N):
            self.gamma_x[i] /= self.scene.boxes[i].m
            self.sum_w[i] /= self.scene.boxes[i].I
    ...
    def PGS(self):
        ...
        while(self.limitExceeded[None] > 1e-8):
            ...
            for k in range(self.nc[None]):
                ...
                if i >= 0:
                    m1_i = 1./self.scene.boxes[i].m
                    I1_i = 1./self.scene.boxes[i].I
                    sum_va = self.gamma_x[i]
                    sum_wa = self.sum_w[i]

                if j >= 0:
                    m2_i = 1./self.scene.boxes[j].m
                    I2_i = 1./self.scene.boxes[j].I
    ...
    def apply_impulses(self):
        for k in range(self.nc[None]):
            ...
            i, j, n = C.i1, C.i2, C.n1
            if i >= 0:
                ti.atomic_add(self.scene.boxes[i].v, -C.gamma * n / self.scene.boxes[i].m)
                self.scene.boxes[i].ω -= C.gamma * C.r1.cross(n) / self.scene.boxes[i].I
            if j >= 0:
                ti.atomic_add(self.scene.boxes[j].v, C.gamma * n / self.scene.boxes[j].m)
                self.scene.boxes[j].ω += C.gamma * C.r2.cross(n) / self.scene.boxes[j].I
